...
const client = new Stomp.Client({ // ĐÚNG: Khởi tạo instance của Stomp.Client
    ...
    onConnect: () => {
        ...
        if (chatRoomId) {
            client.subscribe(`/topic/chat-room/${chatRoomId}`, (messageOutput: any) => {
    ...
    onStompError: (frame: any) => { // Đã sửa lỗi 4: Thêm type 'any' cho 'frame'
...
client.activate(); // Thay vì client.connect({}, ...), dùng client.activate() để kích hoạt kết nối
    ...
    const handleSendMessage = async (messageText: string) => {
        console.log("onSendMessage in ChatPage.tsx is called with message:", messageText);
        if (!chatRoomId || !userId || !stompClient) return;
        ...
        console.log("After sending message logic (API or WebSocket) - if it reaches here");
    };
    ...
    return (
        <div className="container mx-auto h-screen flex">
            ...
            <div className="flex-1 flex flex-col">
                ...
                <MessageDisplay messages={messages} currentUserId={parseInt(userId)} messageAreaRef={messageAreaRef} />
